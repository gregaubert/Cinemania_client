...
public final class API {
	...
	private static final String SERVER_LOCATION = "http://cinemania.jorgealbaladejo.com/app/index.php";
	...
	/*
	 * /devices/register $device $key return:
	 * {"success":1,"message":"registered"}
	 */
	public static boolean registerDevice(String key) {
		...
		Utilities.Response response = Utilities.post(REGISTER_DEVICE_URL,
				params);
		return response.successful();
	}

	/*
	 * /devices/unregister $device return:
	 * {"success":1,"message":"unregistered"}
	 */
	public static boolean unregisterDevice() {
		...
		Utilities.Response response = Utilities.post(UNREGISTER_DEVICE_URL,
				params);
		return response.successful();
	}

	/*
	 * /games/new $device $data (joins the device to the game!) return:
	 * {"success":1,"game":"123472"}
	 */
	...
	/*
	 * /games/available (shows a list of available games) return: ["12345",
	 * "89083", ...]
	 */
	...
	/*
	 * /games/join $device $game (joins the device to the game) return:
	 * {"playerid":1,"success":1}
	 */
	...
	/*
	 * /games/passturn $device $game $data (calls GCM) for all players return:
	 * {"currentPlayer":"1","turn":2,"success":1}
	 */
	...
	/*
	 * /games/passturn $device $game (calls GCM) for all players return:
	 * {"device":"1","game":1,"notified":1,"success":1}
	 */
	...
	public static GameDataResult gameData(long gameIdentifier) {
		...
		if (!response.successful()) {
			...
			try {
				result.mGameData = response.getJson().getJSONObject("game")
						.getString("data");
			} catch (JSONException e) {
				result.mSuccessful = false;
			}
